...
class BetDatabase:
    ...
    def _add_new_columns_if_needed(self, cursor):
        ...
        try:
            ...
            new_columns = {
                ...
                # Valor real em R$ de lucro/perda (para análise de performance)
                'actual_profit_brl': 'REAL',
                # Descrição do jogo (ex: "Manchester City v Arsenal")
                'event_name': 'TEXT',
                # Odd de saída registrada
                'exit_price': 'REAL',
            }
    ...
    def save_bet(self, bet_id: str, market_id: str, event_id: str, sport: str,
                strategy: str, side: str, selection_id: str, entry_price: float,
                stake: float, liability: float = 0.0, event_name: str = None) -> bool:
        """Salva uma nova aposta (interface simplificada usada pelo Lay Draw e futuras estratégias)."""
    ...
    def close_bet(self, bet_id: str, status: str, profit_loss: float,
                  ...
                  actual_profit_brl: float = None) -> bool:
        ...
        if actual_profit_brl is not None:
            update['actual_profit_brl'] = actual_profit_brl

        success = self.update_bet(bet_id, update)
    ...
    def clear_all_data(self) -> Dict:
        """Limpa todos os dados do banco de dados (apostas, estatísticas, saldo)"""
        try:
            conn = self._get_connection(timeout=30.0)
            cursor = conn.cursor()
            ...
            # Resetar autoincrement (opcional, mas ajuda a manter o banco limpo)
            cursor.execute("DELETE FROM sqlite_sequence WHERE name IN ('daily_stats', 'balance_history')")
            
            conn.commit()
            conn.close()
            
            logger.info(f"Dados limpos: {bets_count} apostas, {stats_count} estatísticas, {balance_count} saldos")
            
            return {
                'success': True,
                'bets_deleted': bets_count,
                'stats_deleted': stats_count,
                'balance_deleted': balance_count
            }
        except Exception as e:
            logger.error(f"Erro ao limpar dados do banco: {e}")
            return {
                'success': False,
                'error': str(e)
            }