...
def get_config():
    ...
    try:
        ...
        max_odd_str = config.get('soccer', 'max_odd', fallback='')
        max_odd = float(max_odd_str) if max_odd_str and max_odd_str.strip() else None
        ...
        return jsonify({
            ...
            'config': {
                ...
                'over_05_odds_max': float(config.get('soccer', 'over_05_odds_max', fallback='1.50')),
                'entry_min_minute': int(config.get('soccer', 'entry_min_minute', fallback='1')),
                'entry_max_minute': int(config.get('soccer', 'entry_max_minute', fallback='45')),
                'check_time_window': config.getboolean('soccer', 'check_time_window', fallback=True),
                'pre_match_enabled': config.getboolean('soccer', 'pre_match_enabled', fallback=True),
                'under_hedge_enabled': config.getboolean('soccer', 'under_hedge_enabled', fallback=False),
                'under_hedge_stake': float(config.get('soccer', 'under_hedge_stake', fallback='15.0')),
                'under_hedge_min_minute': int(config.get('soccer', 'under_hedge_min_minute', fallback='75')),
                'stop_loss_lay_enabled': config.getboolean('soccer', 'stop_loss_lay_enabled', fallback=False),
                'stop_loss_lay_threshold': float(config.get('soccer', 'stop_loss_lay_threshold', fallback='2.0')),
                'entry_max_minute': int(config.get('soccer', 'entry_max_minute', fallback='25')),
                'under_high_hedge_enabled': config.getboolean('soccer', 'under_high_hedge_enabled', fallback=False),
                'under_high_hedge_stake': float(config.get('soccer', 'under_high_hedge_stake', fallback='15.0')),
                'under_high_hedge_min_odd': float(config.get('soccer', 'under_high_hedge_min_odd', fallback='1.15')),
                'under_high_hedge_min_odd_first_entry': float(config.get('soccer', 'under_high_hedge_min_odd_first_entry', fallback='1.08')),
                'under_high_hedge_trigger_minute': int(config.get('soccer', 'under_high_hedge_trigger_minute', fallback='40')),
                'under_high_hedge_end_minute': int(config.get('soccer', 'under_high_hedge_end_minute', fallback='88')),
                'under_high_hedge_max_entries': int(config.get('soccer', 'under_high_hedge_max_entries', fallback='2')),
                'partial_stop_loss_lay_enabled': config.getboolean('soccer', 'partial_stop_loss_lay_enabled', fallback=False),
                'partial_stop_loss_lay_min_minute': int(config.get('soccer', 'partial_stop_loss_lay_min_minute', fallback='85')),
                'partial_stop_loss_lay_stake_pct': float(config.get('soccer', 'partial_stop_loss_lay_stake_pct', fallback='0.5')),
            }
        })
    except Exception as e:
        return jsonify({
...
def save_config():
    ...
    try:
        ...
        if 'max_odd' in data:
            max_odd = data['max_odd']
            if max_odd is not None and max_odd != '':
                config.set('soccer', 'max_odd', str(max_odd))
            else:
                config.set('soccer', 'max_odd', '')
        if 'over_05_odds_min' in data:
            config.set('soccer', 'over_05_odds_min', str(data['over_05_odds_min']))
        if 'over_05_odds_max' in data:
            config.set('soccer', 'over_05_odds_max', str(data['over_05_odds_max']))
        if 'entry_min_minute' in data:
            config.set('soccer', 'entry_min_minute', str(data['entry_min_minute']))
        if 'entry_max_minute' in data:
            config.set('soccer', 'entry_max_minute', str(data['entry_max_minute']))
        if 'check_time_window' in data:
            config.set('soccer', 'check_time_window', str(data['check_time_window']).lower())
        if 'pre_match_enabled' in data:
            config.set('soccer', 'pre_match_enabled', str(data['pre_match_enabled']).lower())
        if 'under_hedge_enabled' in data:
            config.set('soccer', 'under_hedge_enabled', str(data['under_hedge_enabled']).lower())
        if 'under_hedge_stake' in data:
            config.set('soccer', 'under_hedge_stake', str(data['under_hedge_stake']))
        if 'under_hedge_min_minute' in data:
            config.set('soccer', 'under_hedge_min_minute', str(data['under_hedge_min_minute']))
        if 'stop_loss_lay_enabled' in data:
            config.set('soccer', 'stop_loss_lay_enabled', str(data['stop_loss_lay_enabled']).lower())
        if 'stop_loss_lay_threshold' in data:
            config.set('soccer', 'stop_loss_lay_threshold', str(data['stop_loss_lay_threshold']))
        if 'entry_max_minute' in data:
            config.set('soccer', 'entry_max_minute', str(data['entry_max_minute']))
        if 'under_high_hedge_enabled' in data:
            config.set('soccer', 'under_high_hedge_enabled', str(data['under_high_hedge_enabled']).lower())
        if 'under_high_hedge_stake' in data:
            config.set('soccer', 'under_high_hedge_stake', str(data['under_high_hedge_stake']))
        if 'under_high_hedge_min_odd' in data:
            config.set('soccer', 'under_high_hedge_min_odd', str(data['under_high_hedge_min_odd']))
        if 'under_high_hedge_min_odd_first_entry' in data:
            config.set('soccer', 'under_high_hedge_min_odd_first_entry', str(data['under_high_hedge_min_odd_first_entry']))
        if 'under_high_hedge_trigger_minute' in data:
            config.set('soccer', 'under_high_hedge_trigger_minute', str(data['under_high_hedge_trigger_minute']))
        if 'under_high_hedge_end_minute' in data:
            config.set('soccer', 'under_high_hedge_end_minute', str(data['under_high_hedge_end_minute']))
        if 'under_high_hedge_max_entries' in data:
            config.set('soccer', 'under_high_hedge_max_entries', str(data['under_high_hedge_max_entries']))
        if 'partial_stop_loss_lay_enabled' in data:
            config.set('soccer', 'partial_stop_loss_lay_enabled', str(data['partial_stop_loss_lay_enabled']).lower())
        if 'partial_stop_loss_lay_min_minute' in data:
            config.set('soccer', 'partial_stop_loss_lay_min_minute', str(data['partial_stop_loss_lay_min_minute']))
        if 'partial_stop_loss_lay_stake_pct' in data:
            config.set('soccer', 'partial_stop_loss_lay_stake_pct', str(data['partial_stop_loss_lay_stake_pct']))
        ...
        return jsonify({
            'success': True,
            'message': 'Configurações salvas com sucesso! As mudanças serão aplicadas automaticamente no próximo ciclo do bot (sem precisar reiniciar).'
        })
    except Exception as e:
        return jsonify({
            'success': False,
...
def _find_config_file():
    """Retorna o caminho do bot_config.ini ou None."""
    for p in [Path('bot_config.ini'), Path('/app/bot_config.ini'), Path('./bot_config.ini')]:
        if p.exists():
            return p
    return None
...
def strategy_status():
    """Retorna o status (ativo/inativo) de cada estratégia."""
    try:
        ...
        return jsonify({
            'success': True,
            'over_05': config.getboolean('soccer',   'enabled',  fallback=True),
            'lay_draw': config.getboolean('lay_draw', 'enabled',  fallback=False),
        })
    except Exception as e:
...
def strategy_toggle():
    ...
    Body: { "strategy": "over_05" | "lay_draw", "enabled": true | false }
    """
    try:
        data = request.get_json()
        ...
        if strategy not in ('over_05', 'under_goals', 'lay_draw'):
            return jsonify({'success': False, 'message': 'Estratégia inválida'}), 400
        if strategy == 'under_goals':
            strategy = 'over_05'  # compatibilidade
        ...
        section = 'soccer' if strategy == 'over_05' else 'lay_draw'
        if not config.has_section(section):
            config.add_section(section)
        config.set(section, 'enabled', 'true' if enabled else 'false')

        with open(cfg_path, 'w', encoding='utf-8') as f:
            config.write(f)

        label = 'Over 0.5 Gols' if strategy == 'over_05' else 'Lay Draw'
        state = 'ATIVADA' if enabled else 'DESATIVADA'
        return jsonify({'success': True, 'message': f'Estratégia {label} {state} com sucesso!'})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500
...
@app.route('/api/pre-match-trading/status', methods=['GET'])
def get_pre_match_trading_status():
    """Obtém o status atual do trading pré-jogo"""
    try:
        config_paths = [
            Path('bot_config.ini'),
            Path('/app/bot_config.ini'),
        ...
        enabled = config.getboolean('pre_match_trading', 'enabled', fallback=False)
        ...
        return jsonify({
            'success': False,
            'enabled': False,
            'message': f'Erro ao ler status: {str(e)}'
        }), 500

@app.route('/api/pre-match-trading/toggle', methods=['POST'])
def toggle_pre_match_trading():
    """Habilita ou desabilita o trading pré-jogo"""
    try:
        data = request.get_json()
        ...
        if not config.has_section('pre_match_trading'):
            config.add_section('pre_match_trading')
        
        # Atualizar enabled
        config.set('pre_match_trading', 'enabled', str(enabled).lower())
        ...
        return jsonify({
            'success': True,
            'enabled': enabled,
            'message': f'Trading Pré-Jogo {status_text} com sucesso! As mudanças serão aplicadas automaticamente no próximo ciclo do bot.'
        })
    except Exception as e:
        return jsonify({
            'success': False,
...
def clear_database():
    """Limpa todos os dados do banco de dados e cache"""
    try:
        ...
        if not result.get('success'):
            return jsonify({
                'success': False,
                'message': f'Erro ao limpar banco de dados: {result.get("error", "Erro desconhecido")}'
            }), 500
        
        # Limpar arquivos de cache se existirem
        cache_files = [
        ...
        for cache_file in cache_files:
            if cache_file.exists():
                try:
                    ...
                    logger.warning(f"Erro ao deletar cache {cache_file}: {e}")
        
        return jsonify({
            'success': True,
            'message': f'Dados limpos com sucesso! Removidos: {result["bets_deleted"]} apostas, {result["stats_deleted"]} estatísticas, {result["balance_deleted"]} saldos, {cache_deleted} arquivos de cache.',
            'details': {
                'bets_deleted': result['bets_deleted'],
                'stats_deleted': result['stats_deleted'],
                'balance_deleted': result['balance_deleted'],
...
@app.route('/api/jogos-ao-vivo', methods=['GET'])
def get_jogos_ao_vivo():
    """Retorna lista de jogos ao vivo (placar, tempo, odds) via jogos_ao_vivo."""
    try:
        import sys
        app_dir = Path(__file__).resolve().parent
        if str(app_dir) not in sys.path:
            sys.path.insert(0, str(app_dir))
        from jogos_ao_vivo import get_live_games_data
        from betfair_api import BetfairAPI
        api = BetfairAPI()
        if not api.login():
            return jsonify({'success': False, 'error': 'Falha no login'}), 500
        result = get_live_games_data(api=api)
        if 'error' in result:
            return jsonify({'success': False, 'error': result['error']}), 500
        return jsonify({'success': True, 'jogos': result.get('jogos', [])})
    except ImportError as e:
        logger.warning("jogos_ao_vivo não disponível: %s", e)
        return jsonify({'success': False, 'error': 'Módulo jogos_ao_vivo não encontrado. Reconstrua a imagem Docker.'}), 500
    except Exception as e:
